    ...
    <div className="space-y-8 max-w-[1600px] mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          ...
          { label: 'New Today', value: leads.filter(l => new Date(l.created_at!).toDateString() === new Date().toDateString()).length.toString(), icon: UserPlus, color: 'text-blue-500' },
          { label: 'Qualified', value: leads.filter(l => l.status === 'Pre-Qualified' || l.status === 'Pre-Approved').length.toString(), icon: TrendingUp, color: 'text-emerald-500' },
          { label: 'Avg. Lead Score', value: leads.length > 0 ? Math.round(leads.reduce((acc, l) => acc + (l.score || 0), 0) / leads.length).toString() : '0', icon: Star, color: 'text-amber-500' },
        ].map((stat, i) => (
                  ...
                  <tr key={lead.id} className="hover:bg-slate-50 transition-colors group">
                    ...
                    <td className="px-6 py-5">
                      <span className={cn(
                        ...
                        lead.status === 'Pre-Qualified' || lead.status === 'Pre-Approved' ? "bg-emerald-50 text-emerald-600" :
